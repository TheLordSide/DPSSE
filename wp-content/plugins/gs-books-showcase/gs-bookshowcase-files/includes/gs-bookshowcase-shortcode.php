...
function gs_bookshowcase_shortcode( $atts ) {
    ...
    $GLOBALS['gs_bookshowcase_loop'] = new WP_Query(
        ...
        $output = '';
        $output = '<div class="wrap gs_bookshowcase_area '. esc_attr( $theme ) .'">';

            if ( $theme == 'gs_book_theme1') {
                include GSBOOKSHOWCASE_FILES_DIR . '/includes/templates/gs_bookshowcase_structure_1_square.php';
            } else {
                echo('<h4 style="text-align: center;">Select correct Theme or Upgrade to <a href="https://www.gsplugins.com/product/wordpress-books-showcase-plugin" target="_blank">Pro version</a> for more Options<br><a href="https://bookshowcase.gsplugins.com" target="_blank">Chcek available demos</a></h4>');
            }
        $output .= '</div>'; // end wrap
    return $output;
}