    ...
    <form method="post" id="edit_profile" name="contact_form" action="" class="form">
        ...
        <div class="card card-default dashboard-panel">
            <div class="card-header bg-white">
                <h3 class="m-0 pt-2 pb-2"><i class="fa fa-user-edit title-icon color-primary"></i> <?= __('Basic Profile', WPDM_TEXT_DOMAIN); ?></h3>
            </div>
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-6"><div class="form-group"><label for="url"><?php _e( "Title:" , WPDM_TEXT_DOMAIN );?></label><input type="text" class="required form-control" name="wpdm_profile[title]" value="<?php echo get_user_meta($user->ID, '__wppmsg_title', true);?>" id="title" ></div></div>
...
<script>
    jQuery(function ($) {
        $('#edit_profile').on('submit', function (e) {
            ...
            $('#edit_profile_sbtn').html("<i class='fa fa-sync fa-spin'></i> <?= esc_attr__( 'Please Wait...', WPDM_TEXT_DOMAIN ) ?>").attr('disabled','disabled');
            $(this).ajaxSubmit({
                success: function (res) {
                    WPPMSG.notify(res.msg, res.type, '#wpdm-fixed-top-center', 10000);
